# Plan: Fix AES PDF Encryption (lopdf 0.39.0 Bug)

> **STATUS: RESOLVED.**  AES-128 and AES-256 encryption now produce readable PDFs — fixed by the lopdf 0.36 → 0.39 bump (encrypted-PDF interop) and confirmed through lopdf 0.42.0 (pdf-maker/pdf-orchestrator → pdf-dump round-trips, including `--password`).  One narrow caveat remains, separate from this plan: lopdf’s `save_modern()` still mis-encrypts ObjStm-packed strings, so the tools use traditional `save()` for encrypted output (see `pdf-orchestrator/LOPDF_SAVE_MODERN_BUG.md` and the `tests/lopdf_canary.rs` sentinel in pdf-dump).  Retained as historical record.
...
```bash
# AES-256
pdf-maker -o /tmp/test.pdf input.pdf "1" --owner-password "test123" --encryption-algorithm aes256
pdf-dump /tmp/test.pdf
# → "decryption error: the supplied password is incorrect"

# AES-128
pdf-maker -o /tmp/test.pdf input.pdf "1" --owner-password "test123" --encryption-algorithm aes128
pdf-dump /tmp/test.pdf
...
- **pdf-maker**: `--owner-password` / `--user-password` with `--encryption-algorithm aes256|aes128` produces unopenable PDFs
...
| `pdf-maker Cargo.toml` | Same dependency changes |
...
Until this is fixed, users should set `PDF_SEC_DocumentSecurityLevel=none` in their config, or omit password variables entirely.  The orchestrator and pdf-maker should log a warning if AES encryption is requested, noting the known issue.
