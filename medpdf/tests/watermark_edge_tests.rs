...
fn test_watermark_layer_over() {
    ...
    match contents {
        Object::Array(arr) => {
            ...
            assert!(
                arr.len() >= 4,
                "Layer over should have at least 4 content refs, got {}",
                arr.len()
            );
        }
        _ => panic!("Contents should be an array after watermark"),
...
fn test_watermark_valign_descent_bottom() {
    ...
    assert!(
        result.is_ok(),
        "VAlign::DescentBottom should succeed: {:?}",
        result.err()
    );
}
...
fn test_watermark_valign_baseline() {
    ...
    assert!(
        result.is_ok(),
        "VAlign::Baseline should succeed: {:?}",
        result.err()
    );
}
...
fn test_watermark_strikeout_and_underline_combined() {
    let (mut dest_doc, page_id) = setup_dest_doc();
    let params = builtin_font_params("BOTH").strikeout(true).underline(true);
    let result = add_text_params(
        &mut dest_doc,
        page_id,
        &params,
    ...
    assert!(
        result.is_ok(),
        "Combined strikeout+underline should succeed: {:?}",
        result.err()
    );
    ...
    assert!(
        re_count >= 2,
        "Should have at least 2 rectangle ops for strikeout+underline, got {re_count}"
    );
}
...
fn test_watermark_strikeout_with_rotation() {
    ...
    assert!(
        result.is_ok(),
        "Strikeout+rotation should succeed: {:?}",
        result.err()
    );
}
...
fn test_watermark_underline_with_rotation() {
    ...
    assert!(
        result.is_ok(),
        "Underline+rotation should succeed: {:?}",
        result.err()
    );
}
...
fn test_watermark_center_align_with_rotation() {
    ...
    assert!(
        result.is_ok(),
        "Center+rotation should succeed: {:?}",
        result.err()
    );
}
...
fn test_watermark_alpha_zero() {
    let (mut dest_doc, page_id) = setup_dest_doc();
    let params = builtin_font_params("INVISIBLE").color(PdfColor::rgba(0.0, 0.0, 0.0, 0.0));
    let result = add_text_params(
        &mut dest_doc,
        page_id,
        &params,
...
fn test_watermark_alpha_one_no_extgstate() {
    let (mut dest_doc, page_id) = setup_dest_doc();
    let params = builtin_font_params("OPAQUE").color(PdfColor::rgba(0.0, 0.0, 0.0, 1.0));
    let result = add_text_params(
        &mut dest_doc,
        page_id,
        &params,
...
fn test_watermark_alpha_half_creates_extgstate() {
    let (mut dest_doc, page_id) = setup_dest_doc();
    let params = builtin_font_params("SEMI").color(PdfColor::rgba(1.0, 0.0, 0.0, 0.5));
    let result = add_text_params(
        &mut dest_doc,
        page_id,
        &params,
...
fn test_multiple_watermarks_accumulate() {
    ...
    for i in 0..5 {
        ...
        let params = builtin_font_params(&text).position(72.0, 72.0 + (i as f32 * 50.0));
        add_text_params(
            &mut dest_doc,
            page_id,
            &params,
...
fn test_watermark_very_large_font_size() {
    ...
    assert!(
        result.is_ok(),
        "Very large font size should succeed: {:?}",
        result.err()
    );
}
...
fn test_watermark_360_degree_rotation() {
    ...
    assert!(
        result.is_ok(),
        "360 degree rotation should succeed: {:?}",
        result.err()
    );
}
...
fn test_watermark_negative_rotation() {
    ...
    assert!(
        result.is_ok(),
        "Negative rotation should succeed: {:?}",
        result.err()
    );
}
...
fn test_watermark_negative_coordinates() {
    ...
    assert!(
        result.is_ok(),
        "Negative coordinates should succeed: {:?}",
        result.err()
    );
}
...
fn test_watermark_on_page_without_contents() {
    ...
    assert!(
        result.is_ok(),
        "Watermark on page without contents should succeed: {:?}",
        result.err()
    );
}
...
fn test_watermark_on_page_without_resources() {
    ...
    assert!(
        result.is_ok(),
        "Watermark on page without resources should succeed: {:?}",
        result.err()
    );
}
...
fn test_multiple_different_builtin_fonts() {
    ...
    let params3 = AddTextParams::new(
        "Times",
        FontData::BuiltIn("Times-Roman".into()),
        "Times-Roman",
    )
    .font_size(12.0)
    .position(72.0, 600.0);
...
fn test_add_rect_layer_over() {
    let (mut dest_doc, page_id) = setup_dest_doc();
    let params = DrawRectParams::new(10.0, 20.0, 100.0, 50.0).color(PdfColor::RED);
    let result = add_rect(&mut dest_doc, page_id, &params);
    assert!(
        result.is_ok(),
        "add_rect layer_over should succeed: {:?}",
        result.err()
    );
    ...
    match contents {
        Object::Array(arr) => {
            ...
            assert!(
                arr.len() >= 4,
                "Layer over rect should have at least 4 content refs, got {}",
                arr.len()
            );
        }
...
fn test_add_rect_layer_under() {
    let (mut dest_doc, page_id) = setup_dest_doc();
    let params = DrawRectParams::new(10.0, 20.0, 100.0, 50.0).layer_over(false);
    let result = add_rect(&mut dest_doc, page_id, &params);
    assert!(
        result.is_ok(),
        "add_rect layer_under should succeed: {:?}",
        result.err()
    );
    ...
    match contents {
        Object::Array(arr) => {
            assert!(
                arr.len() >= 2,
                "Layer under should have at least 2 content streams"
            );
        }
...
fn test_add_rect_with_alpha() {
    ...
    assert!(
        result.is_ok(),
        "add_rect with alpha should succeed: {:?}",
        result.err()
    );
    ...
    assert!(
        content_str.contains("f\n") || content_str.contains("f\r"),
        "Should contain 'f' (fill) operator"
    );
}
...
fn test_add_rect_opaque_no_extgstate() {
    let (mut dest_doc, page_id) = setup_dest_doc();
    let params = DrawRectParams::new(0.0, 0.0, 50.0, 50.0).color(PdfColor::RED);
    ...
    assert!(
        ...
        "Alpha 1.0 should not produce gs operator"
    );
}
...
fn test_add_line_layer_over() {
    ...
    assert!(
        result.is_ok(),
        "add_line layer_over should succeed: {:?}",
        result.err()
    );
    ...
    match contents {
        Object::Array(arr) => {
            assert!(
                arr.len() >= 4,
                "Layer over line should have at least 4 content refs, got {}",
                arr.len()
            );
        }
        _ => panic!("Contents should be an array"),
...
fn test_add_line_layer_under() {
    let (mut dest_doc, page_id) = setup_dest_doc();
    let params = DrawLineParams::new(0.0, 0.0, 100.0, 100.0).layer_over(false);
    let result = add_line(&mut dest_doc, page_id, &params);
    assert!(
        result.is_ok(),
        "add_line layer_under should succeed: {:?}",
        result.err()
    );
}
...
fn test_add_line_with_alpha() {
    let (mut dest_doc, page_id) = setup_dest_doc();
    let params =
        DrawLineParams::new(0.0, 0.0, 100.0, 100.0).color(PdfColor::rgba(0.0, 1.0, 0.0, 0.3));
    let result = add_line(&mut dest_doc, page_id, &params);
    assert!(
        result.is_ok(),
        "add_line with alpha should succeed: {:?}",
        result.err()
    );
    ...
    assert!(
        ...
        "Should contain 'RG' (stroke color)"
    );
}
...
fn test_rect_line_watermark_combined() {
    ...
    // Add watermark over
    let text_params =
        AddTextParams::new("DRAFT", FontData::BuiltIn("Helvetica".into()), "Helvetica")
            .font_size(48.0)
...
fn test_embedded_font_cache_reuses_font_across_pages() {
    ...
    let font_refs1: Vec<_> = font_dict1
        .iter()
        .filter_map(|(_, v)| v.as_reference().ok())
        .collect();
    let font_refs2: Vec<_> = font_dict2
        .iter()
        .filter_map(|(_, v)| v.as_reference().ok())
        .collect();
    ...
    assert_eq!(
        font_refs1[0], font_refs2[0],
        "Both pages should reference the same font object"
    );
}
...
fn test_embedded_font_cache_separate_entries_for_different_fonts() {
    ...
    assert!(
        font_dict.len() >= 2,
        "Should have at least 2 font entries, got {}",
        font_dict.len()
    );
}
...
fn test_embedded_font_without_cache_duplicates_objects() {
    ...
    assert_ne!(
        font_refs1[0], font_refs2[0],
        "Without shared cache, font objects should differ"
    );
}
...
fn test_embedded_font_data_is_compressed() {
    ...
    let params = AddTextParams::new(
        "Compressed",
        FontData::Embedded(font_arc.clone()),
        "TestFont",
    )
    .font_size(12.0)
    .position(72.0, 72.0);
    ...
    assert!(
        found_compressed,
        "Should find a compressed font file stream"
    );
}
